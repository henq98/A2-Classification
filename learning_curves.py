...
def compute_learning_curve(
    ...
    """
    Manual learning-curve implementation with:
    1) one fixed external test set
    2) one fixed training pool
    3) increasing subsets drawn from that training pool
    4) training and testing error rate only
    """
    ...
    for ratio in train_ratios:
        ...
        subset_size = max(1, int(round(ratio * len(y_pool))))

        for repeat in range(n_repeats):
            # draw subset from fixed training pool
            ...
            train_err = 1.0 - accuracy_score(y_sub, y_sub_pred)
...
def plot_learning_curve(results, model_name, save_path=None, show=True):
